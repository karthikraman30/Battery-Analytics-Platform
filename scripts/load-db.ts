/**
 * Load consolidated CSV data into TimescaleDB
 * Run with: DB_PORT=5433 bun run load-db.ts
 */
...
async function loadData(): Promise<void> {
  console.log('📥 Loading data into TimescaleDB...');
  ...
  const dataDir = path.join(baseDir, 'consolidated_data');
  
  if (!fs.existsSync(dataDir)) {
    console.error('❌ consolidated_data directory not found. Run consolidate.ts first.');
    process.exit(1);
  }
