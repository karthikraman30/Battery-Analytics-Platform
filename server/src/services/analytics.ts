/**
 * Analytics Service - Business logic for data queries
 */
...
export async function getDevices(): Promise<DeviceInfo[]> {
  ...
  `);
  logger.info('Fetched devices', { count: result.rows.length });
  return result.rows;
}
...
export async function getBatteryTimeSeries(
  ...
  logger.debug('Fetching battery time series', { deviceId, groupId, startDate, endDate, granularity });
  
  if (granularity === 'raw') {
    const result = await query<BatteryEvent>(`
    ...
    logger.info('Fetched battery time series', { deviceId, groupId, granularity, rows: result.rows.length });
    return result.rows;
...
export async function getChargingSessions(
  ...
  logger.debug('Fetching charging sessions', { deviceId, groupId, startDate, endDate });
  const result = await query<ChargingSession>(`
    SELECT *
    FROM charging_sessions
    WHERE device_id = $1 AND group_id = $2
    ...
    LIMIT 5000
  `, [deviceId, groupId, startDate, endDate].filter(Boolean));
  logger.info('Fetched charging sessions', { deviceId, groupId, rows: result.rows.length });
  return result.rows;
}
...
export async function getChargingStats(
  ...
  logger.debug('Fetching charging stats', { deviceId, groupId, startDate, endDate });
  ...
  const result = await query<ChargingStats>(`
    SELECT 
      device_id,
    ...
    GROUP BY device_id
  `, params);
  logger.info('Fetched charging stats', { deviceId, groupId, found: !!result.rows[0] });
...
export async function getChargingPatterns(
  ...
  logger.debug('Fetching charging patterns', { deviceId: deviceId || 'all', groupId: groupId || 'all', startDate, endDate });
  ...
  const result = await query<ChargingPattern>(`
    SELECT 
    ...
    ORDER BY 1, 2
  `, params);
  logger.info('Fetched charging patterns', { deviceId: deviceId || 'all', groupId: groupId || 'all', rows: result.rows.length });
  return result.rows;
}
...
export async function getAppUsage(
  ...
  logger.debug('Fetching app usage', { deviceId, groupId, groupBy, limit });
  const result = await query<AppUsageStats>(`
    SELECT 
  ...
  logger.info('Fetched app usage', { deviceId, groupId, rows: result.rows.length });
  return result.rows;
}

export async function getTopApps(limit: number = 20): Promise<AppUsageStats[]> {
  logger.debug('Fetching top apps', { limit });
  const result = await query<AppUsageStats>(`
    SELECT 
      COALESCE(app_name, package_name) as name,
      package_name,
    ...
    LIMIT $1
  `, [limit]);
  logger.info('Fetched top apps', { rows: result.rows.length });
  return result.rows;
}
...
export async function getOverallStats(): Promise<OverallStats> {
  ...
  `);
  logger.info('Fetched overall stats', { 
...
export async function getBatteryBoxPlotStats(groupBy: 'device' | 'group' = 'device'): Promise<BoxPlotStats[]> {
  logger.debug('Fetching battery box plot stats', { groupBy });
  
  if (groupBy === 'group') {
    const result = await query<BoxPlotStats>(`
      SELECT 
        group_id,
        'all' as device_id,
    ...
    `);
    logger.info('Fetched battery box plot stats by group', { rows: result.rows.length });
    return result.rows;
  } else {
    const result = await query<BoxPlotStats>(`
      SELECT 
      ...
      LIMIT 30
    `);
    logger.info('Fetched battery box plot stats by device', { rows: result.rows.length });
    return result.rows;
  }
}
...
export async function getChargingDurationBoxPlot(groupBy: 'device' | 'group' = 'device'): Promise<ChargingDurationStats[]> {
  logger.debug('Fetching charging duration box plot', { groupBy });
  
  if (groupBy === 'group') {
    const result = await query<ChargingDurationStats>(`
      SELECT 
        group_id,
        'all' as device_id,
    ...
    `);
    logger.info('Fetched charging duration box plot by group', { rows: result.rows.length });
    return result.rows;
  } else {
    const result = await query<ChargingDurationStats>(`
      SELECT 
      ...
      LIMIT 30
    `);
    logger.info('Fetched charging duration box plot by device', { rows: result.rows.length });
    return result.rows;
  }
}
...
export async function getGroupStats(): Promise<GroupStats[]> {
  logger.debug('Fetching group statistics');
  const result = await query<GroupStats>(`
    SELECT 
      b.group_id,
    ...
    ORDER BY battery_events DESC
  `);
  logger.info('Fetched group stats', { rows: result.rows.length });
  return result.rows;
}
...
export async function getHourlyUsagePatterns(deviceId?: string, groupId?: string): Promise<HourlyUsagePattern[]> {
  logger.debug('Fetching hourly usage patterns', { deviceId, groupId });
  ...
  const result = await query<HourlyUsagePattern>(`
    ...
    ORDER BY 1
  `, params);
  logger.info('Fetched hourly usage patterns', { rows: result.rows.length });
  return result.rows;
}

export async function getBatteryDistribution(deviceId?: string, groupId?: string): Promise<any[]> {
  logger.debug('Fetching battery distribution', { deviceId: deviceId || 'all', groupId: groupId || 'all' });
  ...
  const result = await query(`
    ...
    ORDER BY 1
  `, params);
  logger.info('Fetched battery distribution', { rows: result.rows.length });
  return result.rows;
}
...
export async function getChargingByHour(): Promise<ChargingByHour[]> {
  logger.debug('Fetching charging by hour');
  const result = await query<ChargingByHour>(`
    SELECT 
    ...
    ORDER BY 1
  `);
  logger.info('Fetched charging by hour', { rows: result.rows.length });
  return result.rows;
}
...
export async function getAppBatteryImpact(limit = 20): Promise<AppBatteryImpact[]> {
  logger.debug('Fetching app battery impact', { limit });
  const result = await query<AppBatteryImpact>(`
    SELECT 
    ...
    LIMIT $1
  `, [limit]);
  logger.info('Fetched app battery impact', { rows: result.rows.length });
  return result.rows;
}
...
export async function getDailyUsageSummary(deviceId?: string, groupId?: string): Promise<DailyUsageSummary[]> {
  logger.debug('Fetching daily usage summary', { deviceId, groupId });
  ...
  const result = await query<DailyUsageSummary>(`
    ...
    daily_apps AS (
      SELECT 
        ...
        SUM(duration_seconds) / 3600.0 as total_screen_time_hours,
        COUNT(DISTINCT COALESCE(app_name, package_name)) as unique_apps
      FROM app_usage_events
      WHERE is_screen_off = FALSE ${andClause}
      GROUP BY 1
    )
    SELECT 
      COALESCE(b.date, c.date, a.date)::text as date,
      COALESCE(a.total_screen_time_hours, 0)::numeric(5,1) as total_screen_time_hours,
      COALESCE(c.charging_sessions, 0) as charging_sessions,
      COALESCE(b.avg_battery_level, 0) as avg_battery_level,
      COALESCE(a.unique_apps, 0) as unique_apps
    FROM daily_battery b
    FULL OUTER JOIN daily_charging c ON b.date = c.date
    FULL OUTER JOIN daily_apps a ON COALESCE(b.date, c.date) = a.date
    ORDER BY 1
  `, params);
  logger.info('Fetched daily usage summary', { rows: result.rows.length });
  return result.rows;
}
...
export async function getUserBehaviorClusters(): Promise<UserBehaviorCluster[]> {
  logger.debug('Fetching user behavior clusters');
  const result = await query<UserBehaviorCluster>(`
    WITH device_charging AS (
      SELECT 
        device_id, group_id,
        AVG(start_battery_level)::numeric(5,1) as avg_charge_level,
        COUNT(*)::numeric / NULLIF(MAX(session_start::date) - MIN(session_start::date) + 1, 0) as charge_frequency,
        AVG(duration_minutes)::numeric(5,1) as avg_session_duration
      FROM charging_sessions
      WHERE is_complete = TRUE
      GROUP BY device_id, group_id
    ),
    ...
    SELECT 
      ...
      c.group_id,
      c.avg_charge_level,
      c.charge_frequency::numeric(5,2),
      c.avg_session_duration,
      COALESCE(a.screen_time_hours, 0)::numeric(5,1) as screen_time_hours,
  ...
  `);
  logger.info('Fetched user behavior clusters', { rows: result.rows.length });
  return result.rows;
}
...
export async function getAppBatteryDrain(
  ...
  logger.debug('Fetching app battery drain', { sortBy, limit, deviceId, groupId });
  ...
  const result = await query<AppBatteryDrain>(`
  ...
  logger.info('Fetched app battery drain', { rows: result.rows.length });
  return result.rows;
}
...
export async function getAppUsageByHour(topN = 5): Promise<AppUsageByHour[]> {
  logger.debug('Fetching app usage by hour', { topN });
  
  const result = await query<AppUsageByHour>(`
    WITH top_apps AS (
  ...
  logger.info('Fetched app usage by hour', { rows: result.rows.length });
  return result.rows;
}
