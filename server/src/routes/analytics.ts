...
export const analyticsRoutes = new Elysia({ prefix: '/api/analytics' })
  ...
  .get('/battery-timeseries', async ({ query }) => {
    ...
    const data = await analytics.getBatteryTimeSeries(
      device_id,
      group_id,
      start_date,
      end_date,
  ...
  .get('/charging-sessions', async ({ query }) => {
    ...
    const data = await analytics.getChargingSessions(device_id, group_id, start_date, end_date);
    return { data, count: data.length };
  }, {
    query: t.Object({
      device_id: t.String(),
  ...
  .get('/charging-stats', async ({ query }) => {
    ...
    const rawData = await analytics.getChargingStats(device_id, group_id);
  ...
  .get('/charging-patterns', async ({ query }) => {
    const { device_id, group_id } = query;
    const rawData = await analytics.getChargingPatterns(device_id, group_id);
  ...
  .get('/app-usage', async ({ query }) => {
    ...
    const data = await analytics.getAppUsage(
      device_id,
      group_id,
  ...
  .get('/battery-distribution', async ({ query }) => {
    const { device_id, group_id } = query;
    const data = await analytics.getBatteryDistribution(device_id, group_id);
    return { data };
  }, {
    query: t.Optional(t.Object({
      device_id: t.Optional(t.String()),
