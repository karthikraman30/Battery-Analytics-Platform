...
export const groupedAggregateRoutes = new Elysia({ prefix: '/api/grouped/aggregates' })
  
  .get('/stats', async () => {
    const rawData = await groupedAnalytics.getOverallStats();
    const data = parseNumericStats(rawData as unknown as Record<string, unknown>);
    return { data };
  })
  
  .get('/charging-patterns', async () => {
    const rawData = await groupedAnalytics.getChargingPatterns();
  ...
  .get('/top-apps', async ({ query }) => {
    ...
    const data = await groupedAnalytics.getTopApps(limit);
  ...
  .get('/battery-distribution', async () => {
    const rawData = await groupedAnalytics.getBatteryDistribution();
  ...
  .get('/battery-boxplot', async ({ query }) => {
    ...
    const rawData = await groupedAnalytics.getBatteryBoxPlotStats(groupBy);
  ...
  .get('/charging-duration-boxplot', async ({ query }) => {
    const groupBy = (query.group_by as 'device' | 'group') || 'device';
    const rawData = await groupedAnalytics.getChargingDurationBoxPlot(groupBy);
  ...
  .get('/groups', async () => {
    const rawData = await groupedAnalytics.getGroupStats();
  ...
  .get('/hourly-patterns', async ({ query }) => {
    ...
    const rawData = await groupedAnalytics.getHourlyUsagePatterns(device_id, group_id);
  ...
  .get('/charging-by-hour', async () => {
    const rawData = await groupedAnalytics.getChargingByHour();
  ...
  .get('/app-impact', async ({ query }) => {
    const limit = query.limit ? parseInt(query.limit) : 20;
    const rawData = await groupedAnalytics.getAppBatteryImpact(limit);
    const data = rawData.map(item => ({
  ...
  .get('/app-battery-drain', async ({ query }) => {
    ...
    const limit = query.limit ? parseInt(query.limit) : 20;
    const rawData = await groupedAnalytics.getAppBatteryDrain(sortBy, limit, query.device_id, query.group_id);
    const data = rawData.map(item => ({
      app_name: String(item.app_name),
      sessions: Number(item.sessions),
      devices: Number(item.devices),
  ...
  .get('/app-usage-by-hour', async ({ query }) => {
    ...
    const rawData = await groupedAnalytics.getAppUsageByHour(topN);
    const data = rawData.map(item => ({
      hour: Number(item.hour),
      app_name: String(item.app_name),
  ...
  .get('/user-behaviors', async () => {
    const rawData = await groupedAnalytics.getUserBehaviorClusters();
    const data = rawData.map(item => ({
      device_id: String(item.device_id),
      group_id: String(item.group_id),
